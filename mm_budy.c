...
static int update_max_ptr(uint8_t *new_value) {
    max_ptr = mem_heap_hi();
  if (new_value > max_ptr) { // if new_value > mem_heap_hi()
    if (mem_sbrk(new_value - max_ptr) == -1) { // if mem_sbrk(new_value - mem_heap_hi()) == (void *)-1
      return 0;
    }  // max_ptr = mem_heap_hi()
  }
  return 1;
...
int mm_init(void)
{
  base_ptr = max_ptr = (uint8_t *)mem_sbrk(0);
...
void *mm_malloc(size_t request) {
  ...
  while (bucket + 1 != 0) {
     ...
     * This involves flipping o
     * ur parent's "is split" bit because that bit is
    ...
    // printf("line 424, pop list ptr : %p, size : %d\n", ptr, request);
    /*
  ...
  for(int i = MIN_ALLOC_LOG2; i <= MAX_ALLOC_LOG2; i++){
    list_t *curr = &buckets[i];
    list_t *start = curr;
    curr = curr->next;
    while(curr != start){
      printf("block addr :%p bucket_size: %d\n", curr, 1<<i);
      curr = curr->next;
    } 
    printf("\n");
  }
  printf("I am out while loop! \n");
  return NULL;
}
